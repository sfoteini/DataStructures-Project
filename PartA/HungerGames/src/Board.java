...
public class Board {
	...
	Board(Board b) {
		...
		F = b.F;
		T = b.T;
		weaponAreaLimits = b.weaponAreaLimits;
		foodAreaLimits = b.foodAreaLimits;
		trapAreaLimits = b.trapAreaLimits;
		weapons = b.weapons;
		food = b.food;
		traps = b.traps;		
	}
	...
	/**
	 * The getStringRepresentation() method returns a string array that represents the board 
	 * of the game. The cells of the array that don't have any object are represented by "___".
	 * The cells that have a weapon are represented by "W + playerId + WeaponId" 
	 * and the cells that have a food supply or a trap are represented by "F + FoodId" 
	 * or "T + TrapId" respectively.
	 * @return a string array that represents the board of the game.
	 */
	public String[][] getStringRepresentation(){
		String[][] stringBoard = new String[N][M];
		
		// First, fill the array with "___".
		for(int i = 0; i < N; i++) {
			for(int j = 0; j < M; j++) {
				stringBoard[i][j] = "___";
			}
		...
		// Check the weapons.
		for(int i = 0; i < W; i++) {
			switch(weapons[i].getX()) {
			case -2:
				switch(weapons[i].getY()) {
				case -2:								// (x, y) = (-2, -2)
					stringBoard[N/2 - 2][M/2 - 2] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
				
				case -1:								// (x, y) = (-2, -1)
					stringBoard[N/2 - 1][M/2 - 2] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
					
				case 1:									// (x, y) = (-2, 1)
					stringBoard[N/2][M/2 - 2] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
			...
			case 2: 
				switch(weapons[i].getY()) {
				case -2:								// (x, y) = (2, -2)
					stringBoard[N/2 - 2][M/2 + 1] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
					
				case -1:								// (x, y) = (2, -1)
					stringBoard[N/2 - 1][M/2 + 1] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
					
				case 1:									// (x, y) = (2, 1)
					stringBoard[N/2][M/2 + 1] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
			...
			case 1:
				switch(weapons[i].getY()) {
				case -2:								// (x, y) = (1, -2)
					stringBoard[N/2 - 2][M/2] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
					
				case -1:								// (x, y) = (1, -1)
					stringBoard[N/2 - 1][M/2] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
					
				case 1:									// (x, y) = (1, 1)
					stringBoard[N/2][M/2] = "W" + weapons[i].getPlayerId() + weapons[i].getId();
					break;
		...
		// If the size of board (N = M) is greater than or equal to 6, check for food.
		if(N >= 6) {
			for(int i = 0; i < F; i++) {
				switch(food[i].getX()) {
				case -3:
					switch(food[i].getY()) {
					case -3:							// (x, y) = (-3, -3)
						stringBoard[N/2 - 3][M/2 - 3] = "F" + food[i].getId() + " ";
						break;
						
					case -2:							// (x, y) = (-3, -2)
						stringBoard[N/2 - 2][M/2 - 3] = "F" + food[i].getId() + " ";
						break;
						
					case -1:							// (x, y) = (-3, -1)
						stringBoard[N/2 - 1][M/2 - 3] = "F" + food[i].getId() + " ";
						break;
						
					case 1:								// (x, y) = (-3, 1)
						stringBoard[N/2][M/2 - 3] = "F" + food[i].getId() + " ";
						break;
				...
				case -2:
					switch(food[i].getY()) {
					case -3:							// (x, y) = (-2, -3)
						stringBoard[N/2 - 3][M/2 - 2] = "F" + food[i].getId() + " ";
						break;
						
					case 3:								// (x, y) = (-2, 3)
						stringBoard[N/2 + 2][M/2 - 2] = "F" + food[i].getId() + " ";
						break;
					}
					break;
				...
				case -1:
					switch(food[i].getY()) {
					case -3:							// (x, y) = (-1, -3)
						stringBoard[N/2 - 3][M/2 - 1] = "F" + food[i].getId() + " ";
						break;
						
					case 3:								// (x, y) = (-1, 3)
						stringBoard[N/2 + 2][M/2 - 1] = "F" + food[i].getId() + " ";
						break;
					}
					break;
				...
				case 1:
					switch(food[i].getY()) {
					case -3:							// (x, y) = (1, -3)
						stringBoard[N/2 - 3][M/2] = "F" + food[i].getId() + " ";
						break;
						
					case 3:								// (x, y) = (1, 3)
						stringBoard[N/2 + 2][M/2] = "F" + food[i].getId() + " ";
						break;
					}
					break;
				...
				case 3:
					...
					case -1:							// (x, y) = (3, -1)
						stringBoard[N/2 - 1][M/2 + 2] = "F" + food[i].getId() + " ";
						break;
						
					case 1:								// (x, y) = (3, 1)
						stringBoard[N/2][M/2 + 2] = "F" + food[i].getId() + " ";
						break;
		...
		// If the size of board is greater than or equal to 8, check for traps.
		if(N >= 8) {
			for(int i = 0; i < T; i++) {
				switch(traps[i].getX()) {
				case -4:
					switch(traps[i].getY()) {
					case -4:							// (x, y) = (-4, -4)
						stringBoard[N/2 - 4][M/2 - 4] = "T" + traps[i].getId() + " ";
						break;
						
					case -3:							// (x, y) = (-4, -3)
						stringBoard[N/2 - 3][M/2 - 4] = "T" + traps[i].getId() + " ";
						break;
						
					case -2:							// (x, y) = (-4, -2)
						stringBoard[N/2 - 2][M/2 - 4] = "T" + traps[i].getId() + " ";
						break;
						
					case -1:							// (x, y) = (-4, -1)
						stringBoard[N/2 - 1][M/2 - 4] = "T" + traps[i].getId() + " ";
						break;
						
					case 1:								// (x, y) = (-4, 1)
						stringBoard[N/2][M/2 - 4] = "T" + traps[i].getId() + " ";
						break;
				...
				case -3:
					switch(traps[i].getY()) {
					case -4:							// (x, y) = (-3, -4)
						stringBoard[N/2 - 4][M/2 - 3] = "T" + traps[i].getId() + " ";
						break;
						
					case 4:								// (x, y) = (-3, 4)
						stringBoard[N/2 + 3][M/2 - 3] = "T" + traps[i].getId() + " ";
						break;
					}
					break;
				...
				case -2:
					switch(traps[i].getY()) {
					case -4:							// (x, y) = (-2, -4)
						stringBoard[N/2 - 4][M/2 - 2] = "T" + traps[i].getId() + " ";
						break;
						
					case 4:								// (x, y) = (-2, 4)
						stringBoard[N/2 + 3][M/2 - 2] = "T" + traps[i].getId() + " ";
						break;
					}
					break;
				...
				case -1:
					switch(traps[i].getY()) {
					case -4:							// (x, y) = (-1, -4)
						stringBoard[N/2 - 4][M/2 - 1] = "T" + traps[i].getId() + " ";
						break;
						
					case 4:								// (x, y) = (-1, 4)
						stringBoard[N/2 + 3][M/2 - 1] = "T" + traps[i].getId() + " ";
						break;
					}
					break;
				...
				case 1:
					switch(traps[i].getY()) {
					case -4:							// (x, y) = (1, -4)
						stringBoard[N/2 - 4][M/2] = "T" + traps[i].getId() + " ";
						break;
						
					case 4:								// (x, y) = (1, 4)
						stringBoard[N/2 + 3][M/2] = "T" + traps[i].getId() + " ";
						break;
					}
					break;
				...
				case 4:
					...
					case -2:							// (x, y) = (4, -2)
						stringBoard[N/2 - 2][M/2 + 3] = "T" + traps[i].getId() + " ";
						break;
						
					case -1:							// (x, y) = (4, -1)
						stringBoard[N/2 - 1][M/2 + 3] = "T" + traps[i].getId() + " ";
						break;
						
					case 1:								// (x, y) = (4, 1)
						stringBoard[N/2][M/2 + 3] = "T" + traps[i].getId() + " ";
						break;
